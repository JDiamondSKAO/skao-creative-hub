...
PAGES,SOURCE_TITLES=revise_polish(PAGES,INTERIOR_META,JIRA,CANTO)
...
def footer():return '''<footer class="footer"><div class="footer-cta"><div class="wrap footer-cta-inner"><div class="footer-cta-copy"><h2>Need new work or a review?</h2><p>Tell us what you need and we’ll agree the scope and timing with you.</p></div><div class="footer-cta-actions">'''+a('request.html','Request creative help <span aria-hidden="true">→</span>','button')+a(JIRA,'Report a problem ↗','footer-quiet')+'''</div></div></div><div class="footer-base"><div class="wrap footer-base-inner"><a class="footer-brand" href="index.html"><img src="images/skao-logo-white.png" width="64" alt="SKA Observatory"><span>Creative Hub</span></a><nav aria-label="Footer">'''+a('resources.html','Templates and assets')+a('faq.html','Common questions')+a('contribute.html','Share material')+a('updates.html','Hub updates')+a(CANTO,'Canto library ↗')+'''</nav></div></div></footer><dialog id="searchDialog" aria-labelledby="searchTitle"><div class="dialog-title"><h2 id="searchTitle">Search the Hub</h2><button id="closeSearch" class="close-button" aria-label="Close search">×</button></div><p class="search-scope">Searches Hub pages and guidance. Photos and video are in <a href="'''+CANTO+'''">Canto ↗</a>.</p><form id="searchForm" role="search"><label class="field">What do you need?<input id="searchInput" type="search" maxlength="120" autocomplete="off" placeholder="Try presentation, logo or request"></label></form><div id="searchSuggest" class="search-suggest"><div id="recentPages" hidden><h3>Recently viewed</h3><div id="recentList" class="suggest-list"></div></div><div><h3>Quick links</h3><div class="suggest-list"><a href="logos.html">Logo files</a><a href="standard-template.html">Slide template</a><a href="colours-and-type.html">Colours</a><a href="fonts.html">Fonts</a><a href="email-signatures.html">Email signature</a><a href="request.html">Request creative help</a><a href="'''+CANTO+'''">Photos and video in Canto ↗</a></div></div></div><p id="searchStatus" role="status" class="status"></p><div id="searchResults" class="search-results"></div><div class="search-fallback"><a href="resources.html">Browse templates and assets →</a><a href="request.html">Ask for help →</a></div></dialog><p id="copyAnnounce" class="visually-hidden" role="status"></p>'''+'<script type="application/json" id="hubTitles">'+json.dumps({id:PAGES[f][0] for id,t,f,g in ROUTES}).replace('</','<\\/')+'</script>'+'''<script src="js/theme.js" defer></script><script src="js/main.js?v=curated-20260924" defer></script>'''
def shell(title,body,current,home=False):
 ...
 return '<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>'+html.escape(title)+' | SKAO Creative Hub</title><link rel="stylesheet" href="css/style.css?v=curated-20260924"><script src="js/boot.js"></script></head><body data-mode="preview">'+header(current)+'<div id="announcement" class="announce" hidden><div class="wrap announce-inner"><span class="announce-tag">New</span><p class="announce-text"><a id="announcementLink" href="updates.html">Hub updates</a></p><button type="button" class="announce-close" aria-label="Dismiss announcement"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div></div><div class="wrap preview-note">Design preview · Guidance is proposed; library links open the current staff services.</div><main id="main-content" class="wrap" tabindex="-1">'+main+'</main>'+footer()+'</body></html>'
...
prod=prod.replace('data-mode="preview"','data-mode="confluence" data-context="$stringEscapeUtils.escapeHtml($contextPath)" data-page-id="$page.id"')
