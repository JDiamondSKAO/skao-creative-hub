...
(async()=>{
 ...
 const pdoc=pd.window.document,destinations=[...pdoc.querySelectorAll('.directory .dir-card')].map(a=>a.getAttribute('href'));
 ...
 const menuLinks=new Set([...nw.document.querySelectorAll('#mainNav .mega a')].map(a=>a.getAttribute('href')));
 for(const f of Object.keys(routes).filter(f=>!['canto-guide.html','resources.html'].includes(f)))assert(menuLinks.has(f),'top menu reaches '+f);
 const bb=nq('button[aria-controls="menu-brand"]');assert.equal(bb.dataset.current,'true');bb.click();assert(!nq('#menu-brand').hidden);
 nq('button[aria-controls="menu-events"]').click();assert(nq('#menu-brand').hidden);assert(!nq('#menu-events').hidden);
 nw.document.dispatchEvent(new nw.KeyboardEvent('keydown',{key:'Escape'}));assert(nq('#menu-events').hidden);
 ...
 pass('Top menu reaches every page in one click, menus open one at a time, sidebars and breadcrumbs share the section groups');
