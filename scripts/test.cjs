...
(async()=>{
 ...
 const links=[...lst.window.document.querySelectorAll('.download-list a')];assert.equal(links.length,1,'off-site download links are dropped');assert(links[0].href.startsWith('http://localhost/download/'));
 assert.equal(tq('.asset-panel .status-pill').textContent,'Available');assert(!tq('.how-to-get'));assert(!tq('.asset-downloads').hidden);lst.window.close();
