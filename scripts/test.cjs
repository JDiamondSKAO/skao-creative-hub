...
(async()=>{
 ...
 assert.equal(hw.document.querySelectorAll('[data-search]').length,1);assert(!hq('#homeSearch'));hq('[data-search]').click();hq('#searchInput').value='presentation';hq('#searchForm').dispatchEvent(new hw.Event('submit',{cancelable:true}));await wait();assert(hq('dialog').open);assert(hq('#searchResults a'));hq('#searchInput').dispatchEvent(new hw.KeyboardEvent('keydown',{key:'Escape',bubbles:true,cancelable:true}));assert(!hq('dialog').open);assert.equal(hw.document.activeElement,hq('[data-search]'));assert(hq('.welcome-band .hero-latest #latestAssets'),'newest files sit in the hero');assert(!hq('.presentation-feature'));assert(!hq('.help-band'),'help lives in the footer, not a second band');assert.equal(hw.document.querySelectorAll('.section-index a').length,6);assert(hq('.quick-links a[href="request.html"]'));hd.window.close();pass('Single navigation search works and restores focus; homepage leads with search and the newest files without repeating sections or help');
 ...
 assert(rdoc.querySelector('.page-band h1'));assert.equal(rdoc.querySelectorAll('.footer-base nav a').length,5);assert(rdoc.querySelector('.footer-help a.button').getAttribute('href').startsWith('/crh/'));assert(rdoc.querySelector('#announcement[hidden]'),'banner stays hidden until an announcement loads');assert(rdoc.querySelector('.page-grid.full-width'));assert(!rdoc.querySelector('.sidebar'));
