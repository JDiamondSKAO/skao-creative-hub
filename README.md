...
### Rebuilding the demo pages

All sub-pages (`docs/brand-guidelines/*`, `docs/templates-assets/*`, etc.) are generated by a Python builder at [`scripts/build.py`](scripts/build.py). The builder stamps out each page from a shared shell (header, nav, footer) plus a per-page body defined in the `PAGES` list.

```bash
python3 scripts/build.py            # rebuild all pages
python3 scripts/build.py --list     # show pages without writing
```

The homepage at `docs/index.html` is hand-written — it's too structurally different (hero, category grid, no sidebar) to fit the builder cleanly. Edit it directly.

To add a new demo page: add a `Page(...)` entry to `PAGES` in `build.py`, run the builder, commit the new HTML file.
